...
class Profiles extends SimpleCache {
  ...
  async getData() {
    if (this.isCacheInvalid()) {
      ...
      try {
        ...
        log('error', `profiles data - ${err && err.message ? err.message : 'error getting data'}`);
        return Promise.reject(
          `profiles data - ${err && err.message ? err.message : 'error getting data'}`
        );
      }
    } else {
  ...
  async getById(profileId) {
    ...
    if (profiles) {
      foundProfile = _.find(profiles, (profile) => profile.id === profileId);
    }
